...
/// </summary>
public class PersistedReservedMoneyCheckers(IMongoDatabase database)
{
    ...
    private async Task<long> PinballReservedTokens(User user) =>
        (await (
            from doc in database.GetCollection<BsonDocument>("misc").AsQueryable()
    ...
    private async Task<long> BadgeBuyOffersReservedTokens(User user) =>
        (await (
            from doc in database.GetCollection<BsonDocument>("badgebuyoffers").AsQueryable()
    ...
    private async Task<long> ItemBuyOffersReservedTokens(User user) =>
        (await (
            from doc in database.GetCollection<BsonDocument>("itembuyoffers").AsQueryable()
