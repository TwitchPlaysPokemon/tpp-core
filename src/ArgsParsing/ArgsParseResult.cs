...
namespace ArgsParsing
{
    ...
    public readonly record struct Success<T>
        (T Result, IImmutableList<string> RemainingArgs);

    public readonly record struct Failure
        (ErrorRelevanceConfidence Relevance, string Error);
    ...
    public readonly record struct ArgsParseResult<T>(
        Success<T>? SuccessResult,
        IImmutableList<Failure>
            Failures)
    {
        /// <summary>
