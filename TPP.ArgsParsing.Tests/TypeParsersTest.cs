...
namespace TPP.ArgsParsing.Tests
{
    public class TypeParsersTest
    {
        ...
        public async Task TestPercentageParser()
        {
            ...
            Assert.AreEqual(12.345678, percentageOver100.AsRatio);
            Assert.AreEqual(100.0, percentageExactly100.AsPercent);
            Assert.AreEqual(1.0, percentageExactly100.AsRatio);
            Assert.AreEqual(0.0, percentageZero.AsPercent);
            Assert.AreEqual(0.0, percentageZero.AsRatio);

            ArgsParseFailure failureNoFloat = Assert.ThrowsAsync<ArgsParseFailure>(() =>
            ...
            Assert.AreEqual("did not recognize 'abc' as a floating point number", failureNoFloat.Message);
