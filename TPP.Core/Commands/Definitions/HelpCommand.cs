...
namespace TPP.Core.Commands.Definitions
{
    public class HelpCommand
    {
        ...
        private readonly Dictionary<string, string> _oldCoreCommandDescriptions = new()
        {
            ...
            ["createbadge"] = "Operators only: Create a badge for a user. Arguments: <username> <Pokemon>",
            ["distributebadge"] = "Operators only: Give a badge out at random to inputters at a certain time. Arguments: <Pokemon> <datetime> <count>",
            ["removebadge"] = "Operators only: Remove a badge from a user's inventory. Arguments: <username> <Pokemon>",
            ...
            ["enablepbrmusic"] = "Moderators only: Enable PBR's game music for the next match only.",
            ["disablepbrmusic"] = "Moderators only: Disable PBR's game music if it has been queued for the next match.",
            ...
            // trusted commands
            ["leaderboardtext"] = "Trusted only: Provide leaderboard data in text form. Arguments: season, start position, end position(optional)",
            ...
            // badges
            ["checkbadge"] = "View statistics about a badge. Argument: <Pokemon>",
            ["buybadge"] = "Buy a badge that is being sold. If no selling badge matches, it will place a buy order on the badge. Arguments: <Pokemon> <quantity> t<tokens> <duration>(s(seconds)/m(minutes)/h(hours)/d(days)/w(weeks))(optional: will not place a buy order if not specified)",
            ["listbuybadge"] = "List all the badge buy offers you have up.",
            ["cancelbuybadge"] = "Cancel all buy orders up for a particular badge. Argument: <Pokemon>",
            ["sellbadge"] = "Sell a badge you own for tokens. If no buy offer matches it, it will remain on sale until sold. Arguments: <Pokemon> <quantity> t<tokens>",
