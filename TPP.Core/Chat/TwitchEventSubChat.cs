...
public partial class TwitchEventSubChat : IWithLifecycle, IMessageSource
{
        ...
        _client.NotificationReceived += (_, notification) =>
            TaskToVoidSafely(_logger, async () =>
            {
                if (notification is ChannelChatMessage chatMessage)
                    ...
                    await ChannelSubscribeReceived(channelSubscribe);
                else if (notification is ChannelSubscriptionMessage channelSubscriptionMessage)
                    await ChannelSubscriptionMessageReceived(channelSubscriptionMessage);
    ...
    private async Task MessageReceived(ChannelChatMessage channelChatMessage)
    {
        ...
        string irc =
            ConstructPrivmsgIrcString(channelChatMessage, emotes); // backwards compatibility, old-core needs this
    ...
    private SubscriptionTier ParseTier(ChannelSubscribe.Tier tier) =>
        tier switch
        {
